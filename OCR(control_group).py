...
# --- 4. 按 '數字.' 標記分割文本的函數 ---
def split_text_by_question_markers(document_text: str, questions_data: List[Dict[str, Any]]) -> Dict[int, str]:
    """
    根據問題標記 (如 '1. ', '2. ') 將 OCR 文本分割成與每個問題對應的片段。
    使用 \s* 允許多種空格情況。
    """
    print("正在按問題標記 ('數字.') 分割 OCR 文本...")
    question_texts = {}
    # --- !!! 使用修正後的正則表達式 !!! ---
    # 將 \s+ 修改為 \s* 以匹配零個或多個空格
    markers = list(re.finditer(r'^(\d+)\.\s*', document_text, re.MULTILINE))
    # --- !!! 正則表達式結束 !!! ---

    if not markers:
        print("警告：在 OCR 文本中未找到任何 '數字.' 格式的題目標記！")
        return {}
    ...
    for i, marker in enumerate(markers):
        try:
            ...
            end_pos = markers[i+1].start() if (i + 1) < num_markers else len(document_text)
            question_segment = document_text[start_pos:end_pos].strip()
...
if __name__ == "__main__":
    ...
    # --- 按問題標記分割 OCR 文本 ---
    question_segments = split_text_by_question_markers(document_text, parsed_questions)
